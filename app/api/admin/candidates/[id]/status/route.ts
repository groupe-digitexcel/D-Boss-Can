...
export async function PATCH(
  ...
  if (b.final_decision === 'APPROVED') {
    ...
    if (
      !c ||
      required.some(
        k => (c as unknown as Record<string, unknown>)[k] !== 'VERIFIED'
      )
    ) {
      return NextResponse.json(
        {error: 'Approval blocked: every mandatory verification must be VERIFIED.'},
        {status: 422}
      );
    }
    ...
    const must = ['ID_CARD', 'DRIVER_LICENSE', 'RESIDENCE_PROOF'];

    if (
      !docs ||
      must.some(
        t => !docs.some(
          d => d.document_type === t && d.status === 'VERIFIED'
        )
      )
    ) {
      return NextResponse.json(
